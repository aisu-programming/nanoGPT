...
if __name__ == "__main__":
    ...
    with open(raw_path, "rb") as f:
        data = f.read()
    assert len(data) >= 100_000_000, "enwik8 should be at least 100MB"
    print(f"{len(data)=}")

    # Split: 90M train / 5M val / 5M test
    train_bytes = data[          : 90_000_000]
    val_bytes   = data[90_000_000: 95_000_000]
    test_bytes  = data[95_000_000:100_000_000]

    # Convert to uint8 (0~255)
    train_ids = np.frombuffer(train_bytes, dtype=np.uint8)
    val_ids = np.frombuffer(val_bytes, dtype=np.uint8)
    test_ids = np.frombuffer(test_bytes, dtype=np.uint8)
    ...
    print(f"test.bin has {  test_ids.size:,} tokens.")
